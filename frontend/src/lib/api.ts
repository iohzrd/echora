...
export class API {
  ...
  static async getAllVoiceStates(): Promise<VoiceState[]> {
    return this.request(
      "/voice/states",
      {},
      "Failed to fetch all voice states",
    );
  }
  ...
  static async uploadCustomEmoji(
    name: string,
    file: File,
  ): Promise<CustomEmoji> {
    const formData = new FormData();
...
export type WsOutgoingMessage =
  ...
  | {
      message_type: "screen_share_update";
      channel_id: string;
      is_screen_sharing: boolean;
    }
  | {
      message_type: "camera_update";
      channel_id: string;
      is_camera_sharing: boolean;
    }
...
export type WsIncomingMessage =
  ...
  | {
      ...
      data: { user_id: string; avatar_url?: string };
    }
  | {
  ...
  | {
      type: "voice_speaking";
      data: { user_id: string; channel_id: string; is_speaking: boolean };
    }
  | {
      type: "screen_share_updated";
      data: { user_id: string; channel_id: string; is_screen_sharing: boolean };
    }
  | {
      type: "camera_updated";
      data: { user_id: string; channel_id: string; is_camera_sharing: boolean };
    }
  | {
      type: "new_producer";
      data: {
  ...
  | {
      type: "typing";
      data: { user_id: string; channel_id: string; username: string };
    }
...
export class WebSocketManager {
  ...
  sendVoiceStateUpdate(
    channelId: string,
    update: { is_muted?: boolean; is_deafened?: boolean },
  ) {
    this.send({
      message_type: "voice_state_update",
      channel_id: channelId,
